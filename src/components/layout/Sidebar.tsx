  ...
  return <>
       ...
       <nav className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-sidebar/95 backdrop-blur-lg border-t border-sidebar-border">
         <ul className="flex items-center justify-around py-2">
          ...
          const isActive = location.pathname === item.href;
          return <li key={item.href}>
                 <Link to={item.href} className={cn("flex flex-col items-center gap-1 px-4 py-2 rounded-lg transition-colors text-muted", isActive ? "text-primary" : "text-muted-foreground")}>
