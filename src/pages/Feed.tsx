...
const Index = () => {
  ...
  const savedAlbums = useMemo(() => {
    ...
    return [...savedIds].reverse().map(id => mockAlbums.find(a => a.id === id)).filter(Boolean) as typeof mockAlbums;
  }, [savedIds]);
      ...
      case "hot":
        {
          ...
          const seeded = allTakes.filter(t => hotTakeSet.has(t.id) && t.comment && t.comment.trim().length > 0 && !topTakeIds.has(t.id));
          ...
          if (seeded.length < 6) {
            ...
            const computed = allTakes.filter(t => !seededIds.has(t.id) && !topTakeIds.has(t.id) && t.comment && t.comment.trim().length > 0 && (t.rating <= 2 || t.rating >= 5 && t.rating > t.album.averageRating + 0.5)).map(t => {
              ...
              return {
                ...t,
                hotScore: deviation * 2 + polarityBoost + engagementBoost * 0.5
              };
      ...
      <div className="max-w-6xl mx-auto pb-20 lg:pb-0">
        ...
        <section className="mb-8">
          {searchOpen ? <div className="animate-fade-in">
            ...
            </div> : <button onClick={() => setSearchOpen(true)} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg border border-border/50 text-muted-foreground hover:border-border transition-colors text-sm bg-muted">
        ...
        <section className="mb-10">
          ...
          {albumsMode === "saved" && displayedAlbums.length === 0 ? <div className="flex flex-col items-center justify-center py-10 text-center">
            ...
            </div> : <div className="flex gap-4 overflow-x-auto pb-4 -mx-4 px-4 lg:mx-0 lg:px-0 scrollbar-hide">
